...
$worker->onWorkerStart = function ($worker) {
    ...
    $con->onMessage = function ($con, $data) use ($ok) {
        try {
            ...
            echo "[异常] 处理15分钟K线数据时发生错误：" . $e->getMessage() . "\n";
        }
    };

    $con->onClose = function ($con) {
        echo "[断线] 15分钟K线 WebSocket 连接关闭，1秒后重连...\n";
        $con->reConnect(1);
    };

    $con->onError = function ($con, $code, $msg) {
        echo "[错误] 15分钟K线 WebSocket 错误: code=$code, msg=$msg\n";
    };
